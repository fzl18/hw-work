...
define(['vue'],function (vue) {
    const baseURL = "http://bvcrpc.hwelltech.net:88"
    vue.component('hwui-home', {
        template: 
        `
        <div class="layout">
            <Layout>
                ...
                <Content :style="{padding: '0 50px'}">
                    <Breadcrumb :style="{margin: '20px 0'}">
                        <BreadcrumbItem>欢迎使用轻钱包</BreadcrumbItem>
                    </Breadcrumb>
                    <Card dis-hover>
                        <div style="min-height:calc(100vh - 230px);">
                            <Row>
                                    ...
                                    <Card style="background:#f8f8f9">
                                        <p slot="title" style="height:36px">
                                            <!-- <Poptip
                                                ...
                                                title="是否切换至密钥登录?"
                                                @on-ok="logout">
                                                <Button icon="ios-key" size="large" type='success' v-if="$parent.logintype == 1" >密钥登录</Button>
                                            </Poptip> -->
                                            <Button icon = "md-add" size="large" type='info' @click="showModal" v-if="$parent.logintype == 2">添加授信</Button>
                                            <Poptip
                                                confirm
                                                title="是否退出登录?"
                                                @on-ok="logout">
                                                <Button icon="md-log-out" size="large" type='error'>退出登录</Button>
                                            </Poptip>
                                            
                                        </p>
                                        <Row style="line-height:30px">
                                            <Col>地址：{{$parent.account}}</Col>
                                ...
                                <Col span="24" style="margin-bottom:20px" v-if="$parent.logintype == 2">
                                    <Card style="background:rgba(25,190,107,.03)">
                                        <p slot="title">转账</p>
                                        <Row style="line-height:40px">
                                            ...
                                            <Col style="margin:20px 0">                                                
                                                <Input v-model="paymentInfo.amount" style="width:400px;" size="large" type="number">
                                ...
                                <Col span="24" >
                                    <Card style="background:rgba(43,133,228,.03)">
                                        <p slot="title">交易记录</p>
                                        <i-table stripe :columns="columns" :data="transactionList" height="550"></i-table>
            ...
            <Modal
                ...
                <Row style="padding:30px 0 10px 30px;line-height:50px;font-size:15px">
                    ...
                    <Col span="24">
                    授信币种: 
                    <Input type="number" size="large" v-model="num" placeholder="请输入数量" style="width:100px"></Input>
                    <AutoComplete
                        @keyup.native="upper"
                        v-model="symbol"
                        :data="symbolList"
        ...
        methods: {
            //SDA余额
            getSDA() {
                this.axios.post(baseURL, JSON.stringify({
                    "method": "account_info",
                    "params": [
                        {
