...
exports.default = Page({
  ...
  bindNewpwInput: function bindNewpwInput(e) {
    this.setData({
      newpw: e.detail.value
    });
  },
  bindRepwInput: function bindRepwInput(e) {
    this.setData({
      repw: e.detail.value
    });
  },
  ...
  updatePassword: function updatePassword() {
    var _this = this;
    var curUser = wx.getStorageSync('curUser');
    if (this.data.pw == '' || this.data.newpw == '' || this.data.repw == '') {
      wx.showAlert({
        content: '必填项不能为空！'
      });
      return false;
    }
    if (this.data.newpw !== this.data.repw) {
      wx.showAlert({
        content: '两次密码输入不一致！'
      });
      return false;
