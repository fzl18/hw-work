...
function baseToDrops(base) {
  return (new BigNumber(base)).multipliedBy(constants.BaseDrops).toFixed(0)
}
...
function convertMemo(memo) {
  const _memo = {
    Memo: deleteUndefined({
      MemoType: memo.memoType ? convertStringToHex(memo.memoType) : convertStringToHex(memo.memo_type),
      MemoData: memo.memoData ? convertStringToHex(memo.memoData) : convertStringToHex(memo.memo_data)
    })
  }
  return _memo
}
...
function amountToDrops(amount,parameter) {
  validate.amount(amount,parameter)
  let _amount = amount;
  if (typeof amount === 'string') {
    _amount = baseToDrops(amount)
  }
  if (typeof amount === 'object' && amount.currency === constants.BaseCurrency) {
    _amount = baseToDrops(amount.value)
  }
  return _amount;
}
...
function convertPayee(payee) {
  const _payee = {
    Payee:{
      ...
      Amount: amountToDrops(payee.amount,'payee.amount')
    }
  }
  return _payee
}
