...
const DROPS_PER_BASE = new Decimal('1e6');
const MAX_NETWORK_DROPS = new Decimal('1e17');
const MIN_BASE = new Decimal('1e-6')
const MAX_BASE = MAX_NETWORK_DROPS.dividedBy(DROPS_PER_BASE);
...
const AMOUNT_PARAMETERS_DESCRIPTION = `Native values must be described in drops, a million of which equal one TOKEN.This must be an integer number, with the absolute value not exceeding ${MAX_NETWORK_DROPS}.IOU values must have a maximum precision of ${MAX_IOU_PRECISION} significant digits. They are serialized as\na canonicalised mantissa and exponent. The valid range for a mantissa is between ${MIN_IOU_MANTISSA} and \
${MAX_IOU_MANTISSA}.The exponent must be >= ${MIN_IOU_EXPONENT} and <= ${MAX_IOU_EXPONENT}.Thus the largest serializable IOU value is:
${MAX_IOU.toString()}.And the smallest:${MIN_IOU.toString()}`
...
const parsers = {
  string: function (str) {
    ...
    return [new Decimal(str).dividedBy(DROPS_PER_BASE), Currency.BASE];
  },
...
const Amount = makeClass({
  Amount: function (value, currency, issuer) {
    this.value = value || new Decimal('0');
    this.currency = currency || Currency.BASE;
  ...
  statics: {
    ...
    fromParser: function (parser) {
      ...
      const baseValue = drops.dividedBy(DROPS_PER_BASE);
      return new this(baseValue, Currency.BASE);
    }
  },
  assertValueIsValid: function () {
    // zero is always a valid amount value
    if (!this.isZero()) {
      if (this.isNative()) {
        const abs = this.value.abs();
        if (abs.lt(MIN_BASE) || abs.gt(MAX_BASE)) {
          // value is in BASE scale, but show the value in canonical json form
          raiseIllegalAmountError(this.value.times(DROPS_PER_BASE))
        }
      } else {
  ...
  valueString: function () {
    return (this.isNative() ? this.value.times(DROPS_PER_BASE) : this.value)
      .toString();
  },
