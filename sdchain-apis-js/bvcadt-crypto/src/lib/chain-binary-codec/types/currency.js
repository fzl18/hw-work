...
const ISO_REGEX = /^[A-Z0-9]{2,5}$/;
const HEX_REGEX = /^[A-F0-9]{40}$/;
const Constants = require('../../common/constants');

function isoToBytes(iso) {
  const bytes = new Uint8Array(20);
  if (iso !== Constants.BaseCurrency) {
    const isoBytes = iso.split('').map(c => c.charCodeAt(0));
    switch (iso.length) {
      case 2:
        bytes.set(isoBytes, 13);
        break;
      case 3:
        bytes.set(isoBytes, 12);
        break;
      case 4:
        bytes.set(isoBytes, 11);
        break;
      case 5:
...
const Currency = makeClass({
  ...
  statics: {
    init: function () {
      this.BASE = new this(new Uint8Array(20));
    },
    from: function (val) {
      return val instanceof this ? val : new this(bytesFromRepr(val));
    }
  },
  Currency: function (bytes) {
  ...
  classify: function () {
    ...
    for (let i = bytes.length - 1; i >= 0; i--) {
      // if (bytes[i] !== 0 && !(i === 12 || i === 13 || i === 14)) {
        if (bytes[i] !== 0 && !(i === 10 || i === 11 ||i === 12 || i === 13 || i === 14)) {
        onlyISO = false;
        break;
      }
    }
    const lossLessISO = onlyISO && iso !== Constants.BaseCurrency && ISO_REGEX.test(iso);
    this._isNative = onlyISO && _.isEqual(code, [0, 0, 0]);
    this._iso = this._isNative ? Constants.BaseCurrency : lossLessISO ? iso : null;
  },
  toJSON: function () {
    if (this.iso()) {
      return this.iso();
